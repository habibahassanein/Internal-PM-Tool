...
if submitted and query:
    ...
    if cached_results:
        ...
        all_sources = cached_results.get("all_sources", [])
        ...
        agent_steps = []
        tools_used = set()
        all_sources = []  # Collect all sources for citations
    ...
    if final_answer:
        ...
        # Display citations if sources are available
        if all_sources:
            st.markdown("### 📚 Sources")
            ...
            for i, source in enumerate(unique_sources[:10], 1):  # Limit to 10 citations
                ...
                if source_type == "slack":
                    ...
                    title = f"Slack: #{channel} - @{username}"
                else:
                ...
                <div class="citation-card">
                    ...
                    {'<small style="color: #666;">🔗 <a href="' + url + '" target="_blank">' + url[:60] + ('...' if len(url) > 60 else '') + '</a></small>' if url else ''}
        ...
        <div class="search-summary">
            ...
            <p><strong>Sources Found:</strong> {len(all_sources)}</p>
        </div>
        """, unsafe_allow_html=True)
